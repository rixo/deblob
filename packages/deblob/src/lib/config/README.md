...
- `DeblobConfig` (`config.service.ts`) — the authoring contract, all keys
  ...
  `stateless-modules`' readonly half for a codebase without the types; default
...
- `resolveConfig(raw, { root, configPath, flavors, readers })` →
  ...
  `externalLayers` lookup, the build mirror. A `configLoads` entry is validated
  for shape only; whether its file is covered is extraction's to say. This is a
  service, not a model, because it holds port shapes.

...
red a `missed red` naming the check it waits for (`boot.spec.ts` today). A row
is a `Row`: the sentence the reviewer reads, the tree, the config and the checks
it runs. The whole spec is the table and four lines:

...
| A15  | `type Deps = { cwd: string; store: "fs" \| "memory" }` at the file's root, the parameter's type                            | green — RE-RULED 2026-09-26 (was red, 2026-09-25) | a definition is a declaration "of something that exists at run time … A type is not one: where a type may travel, the import rules already say" (canon edited 2026-09-26). Was: the file that imports everything does not become where shared shapes collect — `assembly-driver-only` already keeps them to drivers and assemblies | none needed                                                                                                                                                              |
